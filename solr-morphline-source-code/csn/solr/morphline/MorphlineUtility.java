...
public class MorphlineUtility {
    ...
    private static final String COPY_FIELD = "_copy_";

    private static final Logger LOG = LoggerFactory.getLogger(MorphlineUtility.class);
    ...
    public static Record createRecord(Record record, String mappingFile) throws Exception {
        ...
        Map<String, Set<String>> copySolrFields = findCopyFields(properties, "");
        ...
        for (String key : fields.keySet()) {
            ...
            if (solrKey == null) {
                if (!childKeys.contains(key)) {
                    LOG.error("mapping not found for " + key);
                    continue;
                }

                List<Record> childRecords = createRecord((String) record.getFirstValue(key), properties, key, (String) record.getFirstValue(PRIMARY_KEY), jsonRecord, copySolrFields);
            ...
            for (String copyFieldKey : copySolrFields.keySet()) {
                Set<String> copyFields = copySolrFields.get(copyFieldKey);
                if (copyFields.contains(solrKey)) {
                    jsonRecord.put(copyFieldKey, record.getFirstValue(key));
                }
            }
        }
    ...
    private static Map<String, Set<String>> findCopyFields(Properties properties, String prefix) {
        Map<String, Set<String>> copyFields = new HashMap<>();

        String key;
        if (prefix.length() > 0)
            key = prefix + "." + COPY_FIELD;
        else key = COPY_FIELD;

        for (Object propertyKey : properties.keySet()) {
            String propertyKeyString = propertyKey.toString();
            if (propertyKeyString.startsWith(key + ".")) {
                String copyFieldName = propertyKeyString.substring(key.length() + 1);
                String[] copyFieldValues = properties.get(propertyKey).toString().split(",");
                Set<String> fields = new HashSet<>();
                Collections.addAll(fields, copyFieldValues);
                copyFields.put(copyFieldName, fields);
            }
        }
    ...
    private static List<Record> createRecord(String xmlString, Properties properties, String key, String primaryKey, Record parentRecord, Map<String, Set<String>> copySolrFields) throws Exception {
        ...
        for (Map<String, String> map : xml) {
            ...
            for (String xmlKey : map.keySet()) {
                ...
                if (propertyValue == null) {
                    LOG.error("mapping not found for " + propertyKey);
                    continue;
                }
                ...
                for (String copyFieldKey : copySolrFields.keySet()) {
                    Set<String> copyFields = copySolrFields.get(copyFieldKey);
                    if (copyFields.contains(propertyValue)) {
                        parentRecord.put(copyFieldKey, map.get(xmlKey));
                    }
                }
            }
